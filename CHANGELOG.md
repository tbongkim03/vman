...
- **PowerShell 실행 정책이 `Restricted` 면 셸 연동이 아예 켜지지 않는데 아무도
  알려주지 않았습니다.** 윈도우 기본값이라 아무것도 안 건드린 PC 가 여기 해당합니다.
  `setup` 은 프로필을 심고 "다음 터미널부터 적용됩니다" 라고 했지만 프로필 자체가
  실행되지 않으니 거짓말이었고, `. $PROFILE` 도 같은 이유로 실패합니다.
  이제 `setup` · `reload` · `activate` · `doctor` 가 정책을 확인하고
  `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` 를 안내합니다.
  (Process 범위는 그 창에만 유효하므로 판단에서 제외합니다.)
